...
WATCHES = [
    ...
    #     이 구분을 프롬프트에 명시하지 않으면 AI가 엉뚱한 글을 높게 평가한다.
    {
        ...
        "topic": (
            "'낙원 - 증명' 시즌3에서 배틀마스터로 주간 랭킹 상위권(클리어 타임 단축)에 들기 위한 정보. "
            "사용자는 이미 증명 보스를 6초대에 잡는 상위권 유저이므로, 초보용 가이드는 무가치하다.\n"
            "높게 평가할 것(8~10점): 상위 랭커의 클리어 기록·빌드·스킬코드·영상, 오프닝 딜사이클 최적화, "
            "저가/카운터 패턴에 각성기 꽂는 타이밍, 시즌3 공명·유산 세팅과 특화 스탯 활용, "
            "클리어 타임을 초 단위로 줄이는 구체적 기법, 랭킹작 물량·타이밍 전략.\n"
            "낮게 평가할 것(0~4점): 증명 1~9단계 등 저단계 가이드, 입문·초반부 팁, "
            "시즌1·시즌2 기준 유산 세팅(시즌3에서 체계가 개편되어 무효), "
            "일반 레이드용 각인·아크그리드·보석 세팅(낙원은 풀 보정이라 미적용), 단순 클리어 인증."
        ),
    },
]
